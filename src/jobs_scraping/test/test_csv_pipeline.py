...
pytestmark = pytest.mark.csv_pipeline
...
def test_datetime_is_set_correctly(mock_datetime):
    mock_datetime.now.return_value = DATE_TIME
    csvpipeline = CSVExportPipeline()
    assert csvpipeline.file_name == DATE_TIME.strftime("%m-%d-%Y %H-%M-%S")


def test_file_is_none_when_spider_was_not_opened(csvpipeline):
    assert csvpipeline.file is None


@patch("jobs_scraping.pipelines.CsvItemExporter")
def test_export_item_is_called_with_correct_item(exporter_mock, csvpipeline):
...
def test_close_spider_closes_file_and_finish_exporting(
    datetime_mock, exporter_mock, os_mock
):
    datetime_mock.now.return_value = DATE_TIME
...
@patch("jobs_scraping.pipelines.CsvItemExporter")
@patch("jobs_scraping.pipelines.datetime")
def test_close_spider_removes_file_with_no_data(datetime_mock, exporter_mock, os_mock):
    datetime_mock.now.return_value = DATE_TIME
